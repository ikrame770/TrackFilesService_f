    ...
    public class TransfersController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateTransfer([FromBody] TransferRequest request)
        {
            ...
            var entity = await _context.Entities
                .FirstOrDefaultAsync(e => e.EntityNumber == request.EntityNumber);
            ...
            if (entity.OwnerId != fromUser.Id)
                return Unauthorized(new { message = "🚫 لا يمكنك إحالة ملف لا تملكه." });
        ...
        public async Task<IActionResult> UpdateTransfer(int id, [FromBody] TransferUpdateRequest updateRequest)
        {
            ...
            if (!string.IsNullOrEmpty(updateRequest.ToRole))
                transfer.ToRole = updateRequest.ToRole;

            if (updateRequest.ToUserId.HasValue)
                transfer.ToUserId = updateRequest.ToUserId;

            if (!string.IsNullOrEmpty(updateRequest.Content))
                transfer.Content = updateRequest.Content;
